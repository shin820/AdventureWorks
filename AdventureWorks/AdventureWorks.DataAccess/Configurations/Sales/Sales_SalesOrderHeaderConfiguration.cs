...
namespace AdventureWorks.DataAccess.Configurations.Sales
{
    internal class Sales_SalesOrderHeaderConfiguration : EntityTypeConfiguration<Sales_SalesOrderHeader>
    {
        public Sales_SalesOrderHeaderConfiguration()
        {
            ...
            HasRequired(a => a.Address).WithMany(b => b.BillToSalesOrderHeaders).HasForeignKey(c => c.BillToAddressId); // FK_SalesOrderHeader_Address_BillToAddressID
            HasRequired(a => a.Address1).WithMany(b => b.ShipToSalesOrderHeaders).HasForeignKey(c => c.ShipToAddressId); // FK_SalesOrderHeader_Address_ShipToAddressID
