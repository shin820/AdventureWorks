...
namespace AdventureWorks.DataAccess
{
    public class AdventureWorksContext : DbContext,IDbContext
    {
        ...
        public void SaveChanges<TEntity>(TEntity root) where TEntity : ObjectWithState
        {
            this.Set<TEntity>().Add(root);

            CheckForEntitiesWithoutStateInterface();

            foreach (var entry in this.ChangeTracker.Entries<ObjectWithState>())
            {
        ...
        {
            var entitiesWithoutStateInterface = from e in this.ChangeTracker.Entries()
